...
export default function PrivacyPolicy({ translations }: PrivacyPolicyProps) {
    //import data from md file ../content/privacy-policy.md
    ...
    return (
        <PageLayout
            metadata={{
                title: t('pages.title')['privacy-policy'] ?? '',
            }}>
...
export function getServerSideProps({ locale, params }: any) {
    ...
    try {
        const serviceTranslation = fs.readFileSync(
            path.join(process.cwd(), `content/privacy-policy-${locale}.md`),
            'utf8'
        );
